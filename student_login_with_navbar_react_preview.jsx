...
function StudentLoginForm() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    alert("התחברת (תלמיד/ה)");
  };
  return (
    <section className="bg-white rounded-2xl p-6 ring-1 ring-black/5 shadow-xl">
      <div className="text-center mb-4">
        <h1 className="text-2xl md:text-3xl font-extrabold mb-1" style={{ color: blue }}>התחברות לתלמידים</h1>
        <p className="text-gray-700 font-bold">כניסה עם אימייל וסיסמה או בעזרת Google</p>
      </div>
      <form onSubmit={submit} className="space-y-4 font-bold">
        ...
        <input type="password" placeholder="סיסמה" value={password} onChange={(e) => setPassword(e.currentTarget.value)} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-right" required minLength={6} />
        <button type="submit" className="w-full py-3 rounded-xl font-extrabold text-white shadow-lg hover:opacity-95" style={{ backgroundColor: blue }}>התחבר/י</button>
      </form>
      ...
      <button type="button" className="w-full border border-gray-300 rounded-xl py-3 flex items-center justify-center gap-2 hover:bg-gray-50 font-bold" aria-label="התחבר עם Google" onClick={() => alert("Google OAuth (דמו)")}>
        <img src="https://www.svgrepo.com/show/355037/google.svg" alt="Google" className="w-5 h-5" />
        התחבר עם Google
      </button>
      <p className="text-center text-gray-600 mt-5 font-bold">אין לך חשבון? <a href="#/signup-student" className="hover:underline" style={{ color: orange }}>להרשמה</a></p>
    </section>
  );
}

export default function StudentLoginWithNavbarPreview() {
  return (
